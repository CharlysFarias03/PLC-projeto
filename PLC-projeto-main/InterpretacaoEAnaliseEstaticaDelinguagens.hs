...
-- Exemplo de uso do while: decrementa x até 0
exemploWhile = Seq (Atr "x" (Lit 3))
    (While (Maior (Var "x") (Lit 0))
        (Atr "x" (Som (Var "x") (Lit (-1)))))

-- Para testar no GHCi:
-- at exemploWhile
-- O estado final deve mostrar x = 0
...
int a (Maior t1 t2) e =
    let (v1, e1) = int a t1 e
        (v2, e2) = int a t2 e1
    in case (v1, v2) of
        (Num n1, Num n2) -> if n1 > n2 then (Num 1, e2) else (Num 0, e2)
        _                -> (Erro, e2)
...
instance Show Valor where
   show (Num x) = show x
   show (Str s) = show s
   show (Objeto nomeClasse campos) = "Objeto da classe " ++ nomeClasse ++ " com campos: " ++ show [(c, show v) | (c, v) <- campos]
   show Erro = "Erro"
   show (Fun f) = "Função"
